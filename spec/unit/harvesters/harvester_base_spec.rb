...
describe SearchSolrTools::Harvesters::Base do
  ...
  it 'serializes a hash and adds it to solr in JSON format' do
    ...
    stub_request(:post, 'https://integration.search-solr.apps.int.nsidc.org/solr/nsidc_oai/update?commit=true')
...
.environment][:collection_name], force: true)
  ...
  def stub_update_and_delete(all_count, not_updated_count)
    ...
    stub_request(:get, 'https://integration.search-solr.apps.int.nsidc.org/solr/nsidc_oai/select?q=data_centers:%22test%22&rows=0&wt=ruby')
      .to_return(status: 200, body: all_response, headers: {})
    stub_request(:get, %r{https://integration.search-solr.apps.int.nsidc.org/solr/nsidc_oai/select\?q=last_update:.*AND%20data_centers:%22test%22&rows=0&wt=ruby})
      .to_return(status: 200, body: updated_response, headers: {})
    delete_stub = stub_request(:post, 'https://integration.search-solr.apps.int.nsidc.org/solr/nsidc_oai/update?wt=json')
                  .with(body:    /{"delete":{"query":"last_update:.* AND data_centers:\\"test\\""}}/,
                        headers: { 'Content-Type' => 'application/json' })
                  .to_return(status: 200, body: ''.dup, headers: {})
    commit_stub = stub_request(:post, 'https://integration.search-solr.apps.int.nsidc.org/solr/nsidc_oai/update?wt=json')
                  .with(body:    /{"commit":{}/,
                        headers: { 'Content-Type' => 'application/json' })
                  .to_return(status: 200, body: ''.dup, headers: {})
