...
describe SearchSolrTools::Harvesters::Echo, :skip => "Obsolete harvester, would need to be updated to new status handling method" do
  before :each do
    @harvester = described_class.new 'integration'
  end

  it 'should retrieve records from the ECHO CSW url' do
    stub_request(:get, 'https://api.echo.nasa.gov/catalog-rest/echo_catalog/datasets.echo10?bounding_box=-180,45,180,90&page_num=1&page_size=100')
