...
describe SearchSolrTools::Harvesters::NcdcPaleo, :skip => "Obsolete harvester, would need to be updated to new status handling method" do
  ...
  describe '#get_docs_with_translated_entries_from_ncdc_paleo' do
    check_values = {
      ...
      'dataset_url' => ['https://gis.ncdc.noaa.gov/gptpaleo/catalog/search/resource/details.page?uuid={BA24F713-5035-4A21-8EEA-56C162517572}'],
    ...
    before(:each) do
      allow(described_object).to receive(:get_results).with(
        'https://gis.ncdc.noaa.gov/gptpaleo/csw?getxml={BA24F713-5035-4A21-8EEA-56C162517572}',
  ...
  describe '#ncdc_paleo_url' do
    it 'returns the expected URL' do
      ...
      expect(harvester.ncdc_paleo_url).to eql('https://gis.ncdc.noaa.gov/gptpaleo/csw')
    end
  end

  describe '#get_results_from_ncdc_paleo_url' do
    it 'calls get_results with a valid csw_request' do
      stub_request(:get, 'https://gis.ncdc.noaa.gov/gptpaleo/csw?ElementSetName=full&TypeNames=gmd:MD_Metadata&maxRecords=50&outputFormat=application/xml&request=GetRecords&resultType=results&service=CSW&startPosition=1&version=2.0.2')
      ...
      expect_any_instance_of(SearchSolrTools::Harvesters::Base).to receive(:get_results).with(
        'https://gis.ncdc.noaa.gov/gptpaleo/csw?service=CSW&version=2.0.2&request=GetRecords&TypeNames=gmd:MD_Metadata&ElementSetName=full&resultType=results&outputFormat=application/xml&maxRecords=50&startPosition=1',
        '//csw:Record'
      ).and_return(true)
  ...
  describe '#build_csw_request' do
    it 'returns the expected URL' do
      harvester = described_class.new('dev')
      expect(harvester.build_csw_request).to eql('https://gis.ncdc.noaa.gov/gptpaleo/csw?service=CSW&version=2.0.2&request=GetRecords&TypeNames=gmd:MD_Metadata&ElementSetName=full&resultType=results&outputFormat=application/xml&maxRecords=1000&startPosition=1')
    end
  end
end
