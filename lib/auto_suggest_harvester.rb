...
class AutoSuggestHarvester < HarvesterBase
  ...
  def short_full_split_add_creator(value, count)
    add_docs = []
    value.split(' > ').each do |v|
      add_docs.concat(standard_add_creator(v, count))
    end
    add_docs
  end

  def standard_add_creator(value, count)
    [{ 'add' => { 'doc' => { 'text_suggest' => value, 'source' => 'NSIDC' } } }]
  end
  ...
  def generate_add_hashes(facet_response, fields)
    ...
    facet_response['facet_counts']['facet_fields'].each do |facet_name, facet_values|
      facet_values.each_slice(2).to_a.each do |facet_value|
        new_docs = fields[facet_name][:creator].call(facet_value[0], facet_value[1])
  ...
  def nsidc_fields
    { 'authoritative_id' => { weight: 1, creator: method(:standard_add_creator) },
      'full_title' => { weight: 1, creator: method(:standard_add_creator) },
      'copy_parameters' => { weight: 1, creator: method(:standard_add_creator) },
      'full_platforms' => { weight: 1, creator: method(:short_full_split_add_creator) },
      'full_sensors' => { weight: 1, creator: method(:short_full_split_add_creator) },
